...
async fn test_fill_gap_before_zero_snapshot(pool: PgPool) -> sqlx::Result<()> {
    ...
    println!("\n  Checking balance at known withdrawal block {}...", withdrawal_block);
    
    // Verify balance before withdrawal
    let check_block_before = withdrawal_block - 100;
    let balance_at_check = get_ft_balance(&pool, &archival_network, account_id, token_id, check_block_before as u64).await
        .expect("Should be able to query balance before withdrawal");
    
    println!("    Block {} (before withdrawal): balance = {}", check_block_before, balance_at_check);
    assert_eq!(balance_at_check, balance_before_withdrawal,
    ...
    let balance_after_withdrawal = get_ft_balance(&pool, &archival_network, account_id, token_id, check_block_after as u64).await
        .expect("Should be able to query balance after withdrawal");
    
    println!("    Block {} (after withdrawal): balance = {}", check_block_after, balance_after_withdrawal);
    assert_eq!(balance_after_withdrawal, "0",
        "Balance after withdrawal should be 0");
    
    println!("  ✓ Confirmed withdrawal at block {}: {} -> 0", withdrawal_block, balance_before_withdrawal);
    ...
    println!("  ✓ Binary search found balance change at block {}", found_withdrawal_block);
    
    // Verify binary search found the correct block
    assert_eq!(found_withdrawal_block, withdrawal_block as u64,
    ...
    println!("\n--- First run: fill_gaps from block {} ---", snapshot_block);
    let filled_run1 = fill_gaps(&pool, &archival_network, account_id, token_id, snapshot_block)
        .await
    ...
    assert!(!filled_run1.is_empty(), "First run: should have filled 1 gap (SNAPSHOT at lookback boundary)");
    assert_eq!(filled_run1.len(), 1, "First run: should have filled exactly 1 gap");
    ...
    println!("    Balance: {} -> {}", filled_record.balance_before, filled_record.balance_after);
    
    // Hard assertions: should be a SNAPSHOT at lookback boundary
    assert_eq!(filled_record.block_height, lookback_boundary,
        "First run: should insert SNAPSHOT at lookback boundary (600k blocks before original)");
    assert_eq!(filled_record.balance_before, "0",
        "First run: balance at lookback boundary was 0");
    assert_eq!(filled_record.balance_after, "0",
        "First run: balance at lookback boundary was 0");
    
    println!("  ✓ First run: Inserted SNAPSHOT at lookback boundary (block {})", lookback_boundary);
    
    // ===== SECOND RUN: Fill gaps from the newly inserted SNAPSHOT =====
    println!("\n--- Second run: fill_gaps from block {} ---", lookback_boundary);
    let filled_run2 = fill_gaps(&pool, &archival_network, account_id, token_id, lookback_boundary)
        .await
        .map_err(|e| sqlx::Error::Protocol(format!("fill_gaps error: {}", e)))?;
    ...
    for (i, gap) in filled_run2.iter().enumerate() {
        println!("    Gap {}: Block {} - {} -> {}", 
            i+1, gap.block_height, gap.balance_before, gap.balance_after);
    }
    
    assert!(!filled_run2.is_empty(), "Second run: should have filled gaps");
    
    // Find the withdrawal record (should be at block 178086210)
    let withdrawal_gap = filled_run2.iter()
    ...
    println!("    Balance: {} -> {}", withdrawal_gap.balance_before, withdrawal_gap.balance_after);
    
    // Hard assertions on the withdrawal
    assert_eq!(withdrawal_gap.balance_before, balance_before_withdrawal,
        "Second run: balance before withdrawal should be 3450");
    assert_eq!(withdrawal_gap.balance_after, "0",
        "Second run: balance after withdrawal should be 0");
    
    println!("  ✓ Second run: Found and filled actual withdrawal at block {}", withdrawal_block);
    ...
    println!("\n--- Final balance change chain (total: {}) ---", records.len());
    for (i, r) in records.iter().enumerate() {
        println!("  {}. Block {}: {} -> {} ({})", 
            i+1, r.block_height, r.balance_before, r.balance_after, r.counterparty);
    }
    ...
    let withdrawal_record = records.iter()
        .find(|r| r.block_height == withdrawal_block)
        .expect("Should have withdrawal record");
    assert_eq!(withdrawal_record.balance_before, balance_before_withdrawal, "Withdrawal: before");
    assert_eq!(withdrawal_record.balance_after, "0", "Withdrawal: after");
    
    // Find both SNAPSHOTs
    let snapshots: Vec<_> = records.iter()
        .filter(|r| r.counterparty == "SNAPSHOT")
        .collect();
    assert!(snapshots.iter().any(|s| s.block_height == lookback_boundary), 
        "Should have SNAPSHOT at lookback boundary");
    assert!(snapshots.iter().any(|s| s.block_height == snapshot_block), 
        "Should have original SNAPSHOT");
    
    println!("\n✓ Test passed: Iterative gap filling successfully found complete history");
    println!("  Key records:");
    println!("    Block {} - Withdrawal: {} USDC -> 0", withdrawal_block, balance_before_withdrawal);
    println!("    Block {} - SNAPSHOT: 0 (inserted by first fill_gaps run)", lookback_boundary);
    println!("    Block {} - SNAPSHOT: 0 (original)", snapshot_block);
    println!("\n  Strategy:");
    println!("    Run 1: Looked back 600k blocks from {}, found balance=0, inserted SNAPSHOT at {}", 
        snapshot_block, lookback_boundary);
    println!("    Run 2: Looked back 600k blocks from {}, found balance=3450, filled withdrawal at {}", 
