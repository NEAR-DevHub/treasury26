...
async fn test_fill_gap_to_past_with_insufficient_lookback(pool: PgPool) -> sqlx::Result<()> {
    ...
    println!(
        "✓ Inserted SNAPSHOT with balance_before = balance_after = {} (simulating discovery bug)",
        balance_bd
    );
    ...
    println!(
        "Balance at block {} (600k blocks earlier): {}",
        lookback_block, balance_at_lookback
    );
    ...
    if balance_at_lookback == balance_at_snapshot {
        ...
        println!(
            "⚠ Balance was different at lookback block - test scenario doesn't match production"
        );
    }
    ...
    match result {
        Ok(filled) => {
            ...
            for gap in &filled {
                println!(
                    "    Block {}: {} -> {}",
                    gap.block_height, gap.balance_before, gap.balance_after
                );
            }
        ...
        Err(e) => {
            ...
            println!("Issue:");
            println!(
                "  1. SNAPSHOT created at block {} with balance_before = balance_after",
                snapshot_block
            );
            println!(
                "  2. fill_gap_to_past detects gap (balance_before = {} != 0)",
            ...
            println!(
                "  3. Searches backward 600,000 blocks to block {}",
                lookback_block
            );
            println!(
                "  4. Balance at {} is same as {} (existed before search range)",
                lookback_block, snapshot_block
            );
            println!(
                "  5. Tries to insert transactional record at block {}",
                lookback_block
            );
