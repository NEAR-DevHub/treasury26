...
async fn test_fill_gaps_loop_until_complete(pool: PgPool) -> sqlx::Result<()> {
    ...
    let records = sqlx::query!(
        r#"
        SELECT block_height, balance_before::TEXT as "balance_before!", balance_after::TEXT as "balance_after!", counterparty
        FROM balance_changes
        WHERE account_id = $1 AND token_id = $2
        ORDER BY block_height
        "#,
    ...
    for r in &records {
        let marker = if r.counterparty == "SNAPSHOT" {
            " [SNAPSHOT]"
        } else {
            ""
        };
        println!(
            "  Block {}: {} -> {}{}",
            r.block_height, r.balance_before, r.balance_after, marker
        );
    }

    // Verify chain integrity for non-SNAPSHOT records only
    // SNAPSHOTs mark search boundaries and may not have continuous balances
    let non_snapshot_records: Vec<_> = records
        .iter()
        .filter(|r| r.counterparty != "SNAPSHOT")
        .collect();

    println!(
        "\n=== Verifying chain integrity ({} non-SNAPSHOT records) ===",
        non_snapshot_records.len()
    );

    let mut prev_balance_after: Option<String> = None;
    for r in &non_snapshot_records {
    ...
    println!("✓ Chain integrity verified (non-SNAPSHOT records)");
