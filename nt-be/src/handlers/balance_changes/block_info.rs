...
use near_api::{NetworkConfig, Chain, Reference};
use std::sync::Arc;
...
mod tests {
    ...
    async fn test_query_mainnet_block_timestamp() {
        ...
        assert_eq!(timestamp, 1750097144159145697, "Block 151386339 timestamp should be exactly 1750097144159145697");
    }
    ...
    async fn test_cache_works() {
        ...
        let timestamp1 = get_block_timestamp(
            &state.archival_network,
            151386339,
            Some(&cache),
        ).await.unwrap();
        ...
        let timestamp2 = get_block_timestamp(
            &state.archival_network,
            151386339,
            Some(&cache),
        ).await.unwrap();
