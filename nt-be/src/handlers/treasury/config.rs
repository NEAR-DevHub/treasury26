...
/// Cache key for `get_config` contract reads (shared with tests that seed the cache).
pub fn treasury_config_cache_key(treasury_id: &AccountId, at_before: u64) -> String {
    CacheKey::new("treasury-config")
        .with(treasury_id)
        .with(at_before)
        .build()
}

/// On-chain `get_config` only (cached). Does not touch `treasury_settings` /
/// `monitored_accounts` — callers that already know local branding state use this
/// to avoid redundant round-trips.
pub async fn fetch_contract_treasury_config(
    state: &Arc<AppState>,
    treasury_id: &AccountId,
    at_before: Option<u64>,
) -> Result<TreasuryConfigFromContract, (StatusCode, String)> {
    let at_before = at_before.unwrap_or(0);
    let cache_key = treasury_config_cache_key(treasury_id, at_before);
...
pub async fn fetch_treasury_config(
    ...
    let at_before = at_before.unwrap_or(0);
    let result = fetch_contract_treasury_config(state, treasury_id, Some(at_before)).await?;
