-- Repoint the NocoDB analytics view's public flow metrics at the unified
-- gold ledger. The gold projector now writes gold_treasury_ledger_events
-- directly and gold_public_history_events is no longer maintained; hidden
-- ledger rows (sponsor top-ups, wraps) are excluded so flow metrics keep
-- their visible-activity semantics.
