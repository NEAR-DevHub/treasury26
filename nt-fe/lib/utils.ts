...
/** Compatibility wrappers so redesign screens keep compiling against the new amount library. */
export function formatCurrency(value: number | Big): string {
    return formatFiatValue(value).display;
}

export function formatCurrencyWithSubCent(value: number | Big): string {
    return formatFiatValue(value).display;
}

export function formatSmartAmount(value: number | string | Big): string {
    return formatTokenQuantity(value).display;
}
...
export function formatActivityAmount(amount: string): string {
    return formatTokenQuantity(amount, { signDisplay: "always" }).display;
}

export function formatBalance(
    balance: string | Big,
    decimals: number,
    _displayDecimals?: number,
): string {
    return formatRawTokenQuantity(balance, decimals, { rounding: "down" })
        .display;
}
...
export function formatTokenAmount(
    ...
    return formatRawTokenQuantity(bigIntAmount, tokenDecimals, {
...
export function toBaseUnits(
    ...
    try {
        return baseUnitsFromDecimal(amount ?? 0, decimals);
    } catch {
        return Big(0);
    }
}
