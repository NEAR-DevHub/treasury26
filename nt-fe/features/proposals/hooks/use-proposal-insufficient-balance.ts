...
export function useProposalInsufficientBalance(
  ...
  const { data: tokenData, isLoading: isTokenLoading } = useToken(requiredFunds?.tokenId);
  const { data: tokenBalanceData, isLoading: isTokenBalanceLoading } = useTokenBalance(treasuryId, requiredFunds?.tokenId, tokenData?.network);

  const insufficientBalanceInfo = useMemo((): InsufficientBalanceInfo => {
    if (!requiredFunds || !tokenData || !tokenBalanceData) {
      return { hasInsufficientBalance: false };
    }


    const requiredBig = Big(requiredFunds.amount);
    ...
    if (requiredBig.gt(availableBig)) {
      return {
        ...
        differenceDisplay: formatBalance(requiredBig.sub(availableBig).toString(), tokenData?.decimals || 24),
      };
    }
