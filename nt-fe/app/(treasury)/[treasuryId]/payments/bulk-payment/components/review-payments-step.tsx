...
export function ReviewPaymentsStep({
  ...
  const quotedRecipientFees =
    confidentialPrepare?.fees?.perRecipientFees.length === paymentData.length
  ...
  return (
    <PageCard className="max-w-[600px] mx-auto">
      ...
      >
                ...
                if (selectedTokenData?.price && balance) {
                  try {
                    ...
                    if (balanceFormatted > 0) {
                      estimatedUSDValue =
                        Number(payment.amount) * selectedTokenData.price;
                    }
                  } catch (error) {
                  ...
                  >
                    <div className="flex items-start gap-3">
                      <NumberBadge
                        number={index + 1}
                        variant={
                          payment.validationError ? "error" : "secondary"
                        }
                      />
                          ...
                          <div className="shrink-0">
                            <div className="flex flex-col gap-2 items-end">
                              <div className="flex items-center gap-2">
                                ...
                                <div className="text-right">
                                  <div className="text-sm font-semibold whitespace-nowrap">
                                    {formatTokenDisplayAmount(payment.amount)}{" "}
                                  ...
                                  {confidentialPrepare?.status === "loading" ? (
                                    ...
                                    recipientFee && (
                                      <div className="text-xs text-muted-foreground whitespace-nowrap">
                                        {tPay("networkFee")}:{" "}
                                        {formatTokenDisplayAmount(recipientFee)}{" "}
                                        {selectedToken.symbol}
                                      </div>
                                    )
                                  )}
                              ...
                              <div className="flex items-center gap-3 justify-end">
                                ...
                                >
                                  <Edit2 className="w-4 h-4" /> {tBulk("edit")}
                                </Button>
                                <Button
                                  variant="unstyled"
                                  size="sm"
        ...
        {!isValidatingAccounts && confidentialPrepare?.status === "loading" && (
          <div className="flex items-center justify-between gap-2 text-sm py-3 border-t border-border">
            <div className="flex items-center gap-1 text-muted-foreground">
              <p>{tPay("networkFee")}</p>
              <Tooltip content={tIntents("networkFeeTooltip")} side="top">
                <Info
        ...
        {!isValidatingAccounts && totalNetworkFee && (
          <div className="flex items-center justify-between gap-2 text-sm py-3 border-t border-border">
            ...
            <p>
              {formatTokenDisplayAmount(totalNetworkFee)} {selectedToken.symbol}
            </p>
          </div>
        )}
