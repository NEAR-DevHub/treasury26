import type { BulkPaymentUsageStats, PlanDetails } from "@/lib/api";
 ...
 */
export function BulkPaymentCreditsDisplay({ credits, planDetails }: BulkPaymentCreditsDisplayProps) {
  const { credits_available, credits_used, total_credits } = credits;
  const { batch_payment_credit_limit, period } = planDetails;
  ...
  const periodDisplay = period === "trial" ? "one-time trial" : period;
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm font-medium border-2 py-1 px-2 rounded-lg">
          {isUnlimited ? "Unlimited" : `${batch_payment_credit_limit || total_credits} / ${periodDisplay}`}
        </span>
      </div>
      ...
      {!isUnlimited && (
        <div className="flex items-center justify-between">
          ...
          <Button
            variant={credits_available === 0 ? "default" : "outline"}
            size="sm"
            className="p-3!"
          >
            Upgrade Plan
