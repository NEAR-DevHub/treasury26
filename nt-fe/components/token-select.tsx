...
export default function TokenSelect({
  ...
  const { data: assets = [], isLoading } = useBridgeTokens(!showOnlyOwnedAssets && open);
  ...
  useEffect(() => {
    if (treasuryAssets.length > 0 && !selectedToken && !locked) {
      ...
      setSelectedToken({
        ...
        balanceUSD: firstToken.balanceUSD,
      });
    }
  ...
  const handleNetworkClick = useCallback(
    (item: NetworkListItem) => {
      ...
      const isTreasuryToken = item.residency && item.residency !== "Intents";
      ...
      if (isTreasuryToken) {
        ...
        if (treasuryToken) {
          setSelectedToken({
            ...
            balance: treasuryToken.balance,
            balanceUSD: treasuryToken.balanceUSD,
          });
        }
      } else {
        // Bridge/Intents token
        let balance: { type: "Standard"; total: Big; locked: Big } | undefined;
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col text-left">
                      ...
                      <div className="text-xs text-muted-foreground flex items-center gap-1">
                        <span>
                          {item.residency === "Ft" ? "Fungible Token" :
                           item.residency === "Near" ? "Native Token" : "Intents Token"}
                        </span>
                        {item.residency === "Near" &&
                          item.lockedBalance && (
                            <Tooltip
